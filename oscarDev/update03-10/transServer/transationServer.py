...
def check_valid_command_BSTS(list):
    if list[2] == '':
        dataToAudit = {'trans': list[0], 'command': list[1], 'username': -1, 'stockname': list[3], 'funds': list[4], 'server': "CLT2", 'timestamp': getCurrTimestamp(), 'message' : 'No username input', 'types': 'six'}
...
def commandControl(mydb, mycursor, data, redClient):
    ...
    if dataList[1] == "ADD":
        ...
        if check_valid_command_BSTS(dataList) == "Input error":
            return "Input error"
        if check_valid_user(mycursor, mydb, dataList[2]) == 0:
            dataToAudit = {'trans': list[0], 'command': list[1], 'username': dataList[2], 'stockname': list[3], 'funds': list[4], 'server': "CLT2", 'timestamp': getCurrTimestamp(), 'message' : 'No user data', 'types': 'fourteen'}
            logQueue.put(dataToAudit)
            return "No such user"
        ...
        if check_valid_user(mycursor, mydb, dataList[2]) == 0:
            dataToAudit = {'trans': dataList[0], 'command': dataList[1], 'username': dataList[2], 'server': "CLT2", 'timestamp': getCurrTimestamp(), 'message': 'No user data', 'types': 'fifteen'}
            logQueue.put(dataToAudit)
            return "No such user"
