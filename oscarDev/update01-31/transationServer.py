...
def checkExistTriggers(command, username, stockname): #dataList[1],2,3
    check = "Select * From triggers Where command = %s AND username = %s AND stockname = %s"
    mycursor.execute(check,(command, username, stockname,))
    return mycursor.fetchall()[0][0]
...
def commandControl(data):
    ...
    if dataList[1] == "ADD":
        ...
        #detect any existing triggers.\
        print '1111'
        trigInfo = checkExistTriggers(dataList[1],dataList[2],dataList[3])
        print '1111'
        print (trigInfo[3])
        print '1111'
        return "SET_SELL_AMOUNT"
