    ...
    while True:
        try:
            ...
            if len(queueDic.get(currName)) > 0:
                ...
                del threadDic[currName]
                del queueDic[currName]
                mycursor.close()
                mydb.close()
                break
...
def commandControl(mydb, mycursor, data, redClient):
    ...
    if dataList[1] == "ADD" and len(dataList) == 4:
    ...
    elif dataList[1] == "QUOTE" and len(dataList) == 4:
    ...
    elif dataList[1] == 'BUY' and len(dataList) == 5:
    ...
    elif dataList[1] == "COMMIT_BUY" and len(dataList) == 3:
    ...
    elif dataList[1] == "CANCEL_BUY" and len(dataList) == 3:
        stockname = redClient.rpop('B_'+dataList[2])
        buyDataFromRedis = redClient.hvals('B_'+dataList[2]+'_'+str(stockname)) #data --trans, command, moneyAmount
    ...
    elif dataList[1] == "SELL" and len(dataList) == 5:
    ...
    elif dataList[1] == "COMMIT_SELL" and len(dataList) == 3:
        #trans,command,username,server,types:userCommand-commitBuy(4)
        #dataToAudit = ",".join([data,"CLT2","four"])
        dataToAudit = {'trans': dataList[0], 'command': dataList[1], 'username': dataList[2], 'server': "CLT2", 'timestamp': getCurrTimestamp(), 'types': 'four'}
        logQueue.put(dataToAudit)
    ...
    elif dataList[1] == "CANCEL_SELL" and len(dataList) == 3:
        stockname = redClient.rpop('S_'+dataList[2])
        sellDataFromRedis = redClient.hvals('S_'+dataList[2]+'_'+str(stockname)) #data --trans, command, moneyAmount
    ...
    elif dataList[1] == "DISPLAY_SUMMARY" and len(dataList) == 3:
    ...
    elif dataList[1] == "SET_BUY_AMOUNT" and len(dataList) == 5:
    ...
    elif dataList[1] == "CANCEL_SET_BUY" and len(dataList) == 4:
    ...
    elif dataList[1] == "SET_BUY_TRIGGER" and len(dataList) == 5 and dataList[4] != "":
        #trans,command,username,stockname,stockprice,server,types:userCommand-setBuyTrigger(6)
        #dataToAudit = ",".join([data,"CLT3","six"])
        dataToAudit = {'trans': dataList[0], 'command': dataList[1], 'username': dataList[2], 'stockname': dataList[3], 'funds': dataList[4], 'server': "CLT3", 'timestamp': getCurrTimestamp(), 'types': 'six'}
        logQueue.put(dataToAudit)

        #trans,command,username,stockname,stockprice,server,types:systemEvent-setBuyTrigger(13)
        #dataToAudit = ",".join([data,"HSD3","thirteen"])
        dataToAudit = {'trans': dataList[0], 'command': dataList[1], 'username': dataList[2], 'stockname': dataList[3], 'funds': dataList[4], 'server': "HSD3", 'timestamp': getCurrTimestamp(), 'types': 'thirteen'}
        logQueue.put(dataToAudit)
    ...
    elif dataList[1] == "SET_SELL_AMOUNT" and len(dataList) == 5:
    ...
    elif dataList[1] == "SET_SELL_TRIGGER" and len(dataList) == 5:
    ...
    elif dataList[1] == "CANCEL_SET_SELL" and len(dataList) == 4:
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if dList[1] == "DUMPLOG":
            ...
            # th.start()
            future = executor.submit(recvFromHttp, redClient, threadCount, currName)
            threadDic.update({currName: future})

            threadCount += 1
            queueDic.update({"DUMPLOG":[data]})
            future = executor.submit(recvFromHttp, redClient, threadCount, "DUMPLOG")
            threadDic.update({"DUMPLOG": future})

            # th = threading.Thread(target=recvFromHttp, args=(redClient, threadCount, "DUMPLOG",))
            # th.start()
