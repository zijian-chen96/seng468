...
class TriggerServer(threading.Thread):
    def __init__(self, mydb, dataList2, time):
        threading.Thread.__init__(self)
        self.mydb = mydb
    ...
    def run(self):
        ...
        while command == 'SET_SELL_TRIGGER':
            ...
            if checkIsTrigger(dataList2[2], dataList2[3], command) == 1:
                #check tigger price and quote price ####getTriggerStockPrice(username, stockname, command)
                trigPrice = getTriggerStockPrice(dataList2[2],dataList2[3],command)
                print("trigger price: " + str(trigPrice))
                #check if the set amount is valid
                print(dataList2)
                if trigPrice <= dataList2[4]:


                    commandInfo = dataList2[3] + ',' + dataList2[2] + '\r'
                    ...
                    print(dataList2)
                    numStocks = checkStockAmount(dataList2[2], dataList2[3])
                    print(numStocks)
                    #userHoldPrice = numStocks * Decimal(dataFromQuote[0])
                    numStocksToSell = int(Decimal(dataList2[4]) / Decimal(dataFromQuote[0]))
                    ...
                    if numStocks >= numStocksToSell:
                        ...
                        #dbLogs((dataList[2], dataList[0], 'SELL-TRIGGER-HOLDER', dataList[3], None, numStocksToSell, currFunds, getCurrTimestamp(), None))

                        currFunds = Decimal(checkAcountFunds(dataList2[2]))

                        moneyCanGet = numStocksToSell * Decimal(dataFromQuote[0])
                        newFunds = currFunds + moneyCanGet

                        updateStockAmount(dataList2[2], dataList2[3], (numStocks-numStocksToSell))
                        dbLogs((dataList2[2], dataList2[0], 'SELL', dataList2[3], dataFromQuote[0], dataList2[4], currFunds, getCurrTimestamp(), dataFromQuote[4]))
                        updateFunds(dataList2[2], newFunds)
                        deleteTriggerFromDB(dataList2[2],dataList2[3],command)
        ...
        while command == 'SET_BUY_TRIGGER':
            if checkIsTrigger(dataList2[2], dataList2[3], command) == 1:
                buyStockPrice = getTriggerStockPrice(dataList2[2], dataList2[3], command)
                print(dataList2)

                currFunds =  Decimal(checkAcountFunds(dataList2[2]))
                if holdMoneyFromAcount(dataList2[2], dataList2[4]) == 1:
                    print(dataList2[4])
                    holdMoney = Decimal(dataList2[4])
                    # username, transnumber, command, stockname, stockprice, amount, funds, times, cryptokey
                    dbLogs((dataList2[2], dataList2[0], 'BUY-TRIGGER-HOLDER', dataList2[3], None, holdMoney, currFunds, getCurrTimestamp(), None))

                    while True:
                        dataFromQuote = sendToQuote(dataList2[3] + ',' + dataList2[2] + '\r').split(',')
                        ...
                        if Decimal(dataFromQuote[0]) <= buyStockPrice and Decimal(dataFromQuote[0]) <= holdMoney:
                            ...
                            addToStocksDB((dataList2[2],dataList2[3],stockAmount))
                            # username, transnumber, command, stockname, stockprice, amount, funds, times, cryptokey
                            dbLogs((dataList2[2], dataList2[0], 'BUY', dataList2[3], dataFromQuote[0], dataList2[4], currFunds, getCurrTimestamp(), dataFromQuote[4]))
                            updateFunds(dataList2[2],userFundsLeft)
                            deleteTriggerFromDB(dataList2[2],dataList2[3],command)
                            print('Good!')
                            break
...
def checkIsTrigger(username, stockname, command):
    check = "SELECT count(command) FROM triggers WHERE username = %s AND stockname = %s AND command = %s GROUP BY command"
...
def commandControl(data):
    ...
    if dataList[1] == "ADD":
        ...
        ts = TriggerServer(mydb, dataList2, time)
        ts.start()
        ts.join()
